...
/**
 * Generates all insights from expense data
...
export function generateInsights(
  ...
  const trendInsights = calculateMonthOverMonthTrends(expenses, formatCurrency)
  ...
  // 2. New categories
  const newCategoryInsights = detectNewCategories(expenses, formatCurrency)
  ...
  const patternInsights = detectWeekendWeekdayPatterns(expenses, formatCurrency)
  ...
  const topCategory = findTopSpendingCategory(expenses)
  if (topCategory && topCategory.percentage > 40) {
    results.push({
      ...
      description:
        CATEGORY_TIPS[topCategory.category] ||
  ...
  const spikeAlert = detectSpendingSpikes(expenses, formatCurrency)
  if (spikeAlert) {
    results.push({
      ...spikeAlert,
      icon: icons.AlertCircle,
  ...
  const velocityInsight = detectSpendingVelocity(expenses, formatCurrency)
  if (velocityInsight) {
    results.push({
      ...velocityInsight,
      icon:
